...
def load_system_prompts():
    ...
    try:
        return open(system_prompts_path, encoding="utf-8").read()
...
def load_report_prompts():
    ...
    try:
        return open(report_prompt_path, encoding="utf-8").read()
    except Exception as e:
...
def load_rag_prompts():
    try:
        ...
        logger.error("[load_rag_prompts]在yaml配置项中没有rag_prompts_path配置项")
        raise e

    try:
        return open(rag_prompts_path, encoding="utf-8").read()
    except Exception as e:
